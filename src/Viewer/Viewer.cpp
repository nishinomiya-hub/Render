...
#define CREATE_UNIFORM_BLOCK(name) renderer_->createUniformBlock(#name, sizeof(name))
        ...
        bool Viewer::create(int width, int height, int outTexId)
        {
            ...
            if (!renderer_)
            {
                LOG_ERROR("Viewer::create failed: createRenderer error");
                return false;
            }
        ...
        void Viewer::setupSkybox(ModelMesh &skybox)
        {
            pipelineSetup(skybox, skybox.material->shadingModel,
                          ...
                          {
                              rs.depthFunc = config_.reverseZ ? DepthFunction::GEQUAL : DepthFunction::LEQUAL;
                              rs.depthMask = false;
                          });
        }
        ...
        void Viewer::setupMeshBaseColor(ModelMesh &mesh, bool wireframe)
        {
            pipelineSetup(mesh, Shading_BaseColor,
                          ...
                          [&](RenderStates &rs) -> void
                          {
                              rs.polygonMode = wireframe ? PolygonMode::LINE : PolygonMode::FILL;
                          });
        }
        ...
        bool Viewer::setupShaderProgram(Material &material, ShadingModel shading)
        {
            ...
            {
                LOG_ERROR("setupShaderProgram failed: %s", Material::shadingModelStr(shading));
            }
        ...
        void Viewer::setupPipelineStates(ModelBase &model, const std::function<void(RenderStates &rs)> &extraStates)
        {
            ...
            rs.depthFunc = config_.reverseZ ? DepthFunction::GREATER : DepthFunction::LESS;
        ...
        bool Viewer::initSkyboxIBL()
        {
            ...
            if (texCubeIt == skybox.material->textures.end())
            {
                ...
                if (texEqIt != skybox.material->textures.end())
                {
                    ...
                    LOG_DEBUG("convert equirectangular to cube map: %s.", success ? "success" : "failed");
                    if (success)
                    {
                        textureCube = texCvt;
            ...
            if (!textureCube)
            {
                LOG_ERROR("initSkyboxIBL failed: skybox texture cube not available");
                return false;
            }

            // generate irradiance map
            LOG_DEBUG("generate ibl irradiance map ...");
            ...
            {
                LOG_ERROR("initSkyboxIBL failed: generate irradiance map failed");
                return false;
            }
            LOG_DEBUG("generate ibl irradiance map done.");

            // generate prefilter map
            LOG_DEBUG("generate ibl prefilter map ...");
            ...
            {
                LOG_ERROR("initSkyboxIBL failed: generate prefilter map failed");
                return false;
            }
            LOG_DEBUG("generate ibl prefilter map done.");
        ...
        {
            size_t seed = 0;
            Hash::hashCombine(seed, (int)shading);
            for (auto &def : defines)
            {
                Hash::hashCombine(seed, def);
            }
            return seed;
        }
        ...
        size_t Viewer::getPipelineCacheKey(Material &material, const RenderStates &rs)
        {
            ...
            Hash::hashCombine(seed, (int)material.materialObj->shadingModel);

            // TODO pack together
            Hash::hashCombine(seed, rs.blend);
            Hash::hashCombine(seed, (int)rs.blendParams.Func_RGB);
            Hash::hashCombine(seed, (int)rs.blendParams.Src_RGB);
            Hash::hashCombine(seed, (int)rs.blendParams.Dst_RGB);
            Hash::hashCombine(seed, (int)rs.blendParams.Func_Alpha);
            Hash::hashCombine(seed, (int)rs.blendParams.Src_Alpha);
            Hash::hashCombine(seed, (int)rs.blendParams.Dst_Alpha);

            Hash::hashCombine(seed, rs.depthTest);
            Hash::hashCombine(seed, rs.depthMask);
            Hash::hashCombine(seed, (int)rs.depthFunc);

            Hash::hashCombine(seed, rs.cullFace);
            Hash::hashCombine(seed, (int)rs.primitiveType);
            Hash::hashCombine(seed, (int)rs.polygonMode);

            Hash::hashCombine(seed, rs.lineWidth);
