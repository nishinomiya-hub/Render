...
namespace Learn
{
    ...
    public:
        std::string name_;
        int offset_;
        UniformDesc(std::string name, int offset) : name_(name), offset_(offset) {}
    };
    ...
    public:
        ...
        static inline glm::ivec2 textureSize(Sampler2DSoft<RGBA> *sampler, int lod)
        {
            ...
            return glm::ivec2(buffer->width, buffer->height);
        }
        ...
        float getSampler2DLod(BaseSampler<RGBA> *sampler) const
        {
            ...
            glm::vec2 texSize = glm::vec2(sampler->getWidth(), sampler->getHeight());
