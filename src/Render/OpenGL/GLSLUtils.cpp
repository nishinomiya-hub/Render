...
bool ShaderGLSL::loadSource(const std::string &source) {
  ...
  if (shaderStr.empty()) {
    LOG_ERROR("ShaderGLSL::loadSource failed: empty source");
    return false;
  }
  ...
  if (isCompiled == GL_FALSE) {
    ...
    LOG_ERROR("compile shader failed: %s", &infoLog[0]);
...
bool ShaderGLSL::loadFile(const std::string &path) {
  ...
  if (source.length() <= 0) {
    LOG_ERROR("read shader source failed");
    return false;
  }
...
bool ProgramGLSL::loadShader(ShaderGLSL &vs, ShaderGLSL &fs) {
  ...
  if (isLinked == GL_FALSE) {
    ...
    LOG_ERROR("link program failed: %s", &infoLog[0]);
...
bool ProgramGLSL::loadSource(const std::string &vsSource, const std::string &fsSource) {
  ...
  if (!vs.loadSource(vsSource)) {
    LOG_ERROR("load vertex shader source failed");
    return false;
  }

  if (!fs.loadSource(fsSource)) {
    LOG_ERROR("load fragment shader source failed");
    return false;
  }
...
bool ProgramGLSL::loadFile(const std::string &vsPath, const std::string &fsPath) {
  ...
  if (!vs.loadFile(vsPath)) {
    LOG_ERROR("load vertex shader file failed: %s", vsPath.c_str());
    return false;
  }

  if (!fs.loadFile(fsPath)) {
    LOG_ERROR("load fragment shader file failed: %s", fsPath.c_str());
    return false;
  }
...
void ProgramGLSL::use() const {
  if (id_) {
    ...
    LOG_ERROR("failed to use program, not ready");
  }
}
