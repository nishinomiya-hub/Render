...
namespace Learn
{
    std::shared_ptr<Buffer<RGBA>> Image::readImageRGBA(const std::string &path)
    {
        int iw = 0, ih = 0, n = 0;
        unsigned char *data = stbi_load(path.c_str(), &iw, &ih, &n, STBI_default);
        if (data == nullptr)
        {
            LOG_DEBUG("Image::readImage failed, path: %s", path.c_str());
            return nullptr;
        }
    ...
    void Image::writeImage(char const *filename, int w, int h, int comp, const void *data, int strideInBytes,
                                bool flipY)
    {
    ...
    void Image::convertFloatImage(RGBA *dst, float *src, uint32_t width, uint32_t height)
    {
        float *srcPixel = src;
