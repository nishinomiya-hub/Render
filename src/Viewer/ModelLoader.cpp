...
namespace Learn
{
    namespace View
    {
        ...
        bool ModelLoader::loadSkybox(const std::string &filepath)
        {
            ...
            LOG_DEBUG("load skybox, path: %s", filepath.c_str());
        ...
        bool ModelLoader::loadModel(const std::string &filepath)
        {
            ...
            LOG_DEBUG("load model, path: %s", filepath.c_str());

            // load model
            Assimp::Importer importer;
            if (filepath.empty())
            {
                LOG_ERROR("ModelLoader::loadModel, empty model file path.");
                return false;
            }
            ...
            {
                LOG_ERROR("ModelLoader::loadModel, description: %s", importer.GetErrorString());
                return false;
            }
            ...
            {
                LOG_ERROR("ModelLoader::loadModel, process node failed.");
                return false;
            }
        ...
        bool ModelLoader::processMesh(const aiMesh *ai_mesh, const aiScene *ai_scene, ModelMesh &outMesh)
        {
            ...
            for (size_t i = 0; i < ai_mesh->mNumFaces; i++)
            {
                ...
                {
                    LOG_ERROR("ModelLoader::processMesh, mesh not transformed to triangle mesh.");
                    return false;
                }
                                          ...
                                          Material &material)
        {
            ...
            for (size_t i = 0; i < ai_material->GetTextureCount(textureType); i++)
            {
                ...
                {
                    LOG_WARN("load texture type=%d, index=%d failed with return value=%d", textureType, i, retStatus);
                    continue;
                }
                ...
                case aiTextureType_UNKNOWN:
                    ...
                    break;
                default:
                    //        LOG_WARN("texture type: %s not support", aiTextureTypeToString(textureType));
                ...
                {
                    LOG_ERROR("load texture failed: %s, path: %s", Material::materialTexTypeStr(texType), absolutePath.c_str());
                }
            }
        }
        ...
        std::shared_ptr<Buffer<RGBA>> ModelLoader::loadTextureFile(const std::string &path)
        {
            ...
            LOG_DEBUG("load texture, path: %s", path.c_str());

            auto buffer = Image::readImageRGBA(path);
            if (buffer == nullptr)
            {
                LOG_DEBUG("load texture failed, path: %s", path.c_str());
                return nullptr;
            }
