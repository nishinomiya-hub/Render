...
#define GL_STATE_SET(var, gl_state)   \
    if (var)                          \
        GL_CHECK(glEnable(gl_state)); \
    else                              \
        GL_CHECK(glDisable(gl_state));
...
namespace Learn
{
    ...
    void RendererOpenGL::setPipelineStates(std::shared_ptr<PipelineStates> &states)
    {
        ...
        GL_CHECK(glBlendEquationSeparate(OpenGL::cvtBlendFunction(renderStates.blendParams.Func_RGB),
                                         OpenGL::cvtBlendFunction(renderStates.blendParams.Func_Alpha)));
        GL_CHECK(glBlendFuncSeparate(OpenGL::cvtBlendFactor(renderStates.blendParams.Src_RGB),
                                     OpenGL::cvtBlendFactor(renderStates.blendParams.Dst_RGB),
                                     OpenGL::cvtBlendFactor(renderStates.blendParams.Src_Alpha),
                                     OpenGL::cvtBlendFactor(renderStates.blendParams.Dst_Alpha)));
