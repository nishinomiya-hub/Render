...
namespace Learn
{
    namespace View
    {
                                                  ...
                                                  std::shared_ptr<Texture> &texOut)
        {
            ...
            if (!success)
            {
                LOG_ERROR("create render context failed");
                return false;
            }
      ...
      uniformsBlockPrefilter->setData(&uniformsPrefilter, sizeof(UniformsIBLPrefilter)); });
                                                   ...
                                                   MaterialTexType texType)
        {
            ...
            if (!success)
            {
                LOG_ERROR("create shader program failed");
                return false;
            }
                                         ...
                                         uint32_t texOutLevel, const std::function<void()> &beforeDraw)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                ctx.fbo->setColorAttachment(texOut, CubeMapFace(CubeMapFace_PositiveX + i), texOutLevel);
        ...
        {
            return Hash::getHashMD5(tex->tag + std::to_string(tex->width) + std::to_string(tex->height));
        }
