...
namespace Learn
{
    namespace View
    {
#define CASE_CREATE_SHADER_GL(shading, source)                                    \
    case shading:                                                                 \
        ...
        public:
            ...
            int swapBuffer() override
            {
                ...
                if (texColorMain_->multiSample)
                {
                    ...
                    GL_CHECK(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, texColorMain_->getId(), 0));

                    GL_CHECK(glBindFramebuffer(GL_TEXTURE_2D, outTexId_));
                    ...
                    GL_CHECK(glBindFramebuffer(GL_READ_BUFFER, fbo_in_));
                    GL_CHECK(glBindFramebuffer(GL_DRAW_BUFFER, fbo_out_));
